...
import { Plus, Edit2, Trash2, Loader2, AlertCircle, ArrowLeft, X, Tv, Receipt } from "lucide-react";
...
const EMPTY_FORM: FormData = {
  name: "", category: "", price: "",
  billing_cycle: "monthly", next_billing_date: "", notes: "",
};
...
function AddDialog({
  open, onOpenChange, editItem, onSaved, mode,
}: {
  open: boolean;
  ...
  const availableCats  = editItem
    ? CATEGORIES
    : CATEGORIES.filter(c =>
        isSubscription ? SUBSCRIPTION_CATS.includes(c.value) : BILL_CATS.includes(c.value)
      );
  ...
  useEffect(() => {
    ...
    if (editItem) {
      ...
      setFormData({
        ...
        price: String(editItem.price), billing_cycle: editItem.billing_cycle,
        next_billing_date: editItem.next_billing_date, notes: editItem.notes ?? "",
      });
    } else {
      setStep(1);
  ...
  async function handleSave(e: { preventDefault(): void }) {
    ...
    const payload = {
      name: formData.name.trim(), category: formData.category, price,
      billing_cycle: formData.billing_cycle, next_billing_date: formData.next_billing_date,
  ...
  const stepTitle = editItem
    ? `Edit ${selectedCat ? getCat(editItem.category).label : "item"}`
    : step === 1 ? `Choose category`
    : step === 2 ? selectedCat?.label ?? "Choose service"
    : `Add details`;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={false} className="w-full max-w-sm sm:max-w-md mx-auto max-h-[90vh] flex flex-col gap-0 p-0 overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 pt-5 pb-4 border-b border-gray-100 flex-shrink-0">
          ...
          <DialogTitle className="flex-1 text-base font-bold text-gray-900 capitalize">
            {stepTitle}
          </DialogTitle>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              {availableCats.map(cat => (
                <button
                  ...
                  className={cn("flex flex-col items-start gap-1.5 p-4 rounded-2xl border-2 text-left transition-all active:scale-95", cat.color)}
                >
            ...
            <div className="pt-2 space-y-3">
              ...
              <div className="border-t border-gray-100 pt-3">
                ...
                <div className="flex gap-2">
                  <Input
                    placeholder={`Custom ${selectedCat?.label} name...`}
                    className="h-11 border-gray-200 text-sm"
                    value={formData.name}
                    onChange={e => set("name", e.target.value)}
                    onKeyDown={e => { if (e.key === "Enter" && formData.name) { e.preventDefault(); setStep(3); } }}
                  />
                  <Button type="button" disabled={!formData.name} onClick={() => setStep(3)} className="h-11 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl flex-shrink-0">
                    Next
                  </Button>
                </div>
              </div>
          ...
          {step === 3 && (
            <form id="sub-form" onSubmit={handleSave} className="space-y-4 pt-2">
              ...
              <div>
                ...
                <Input className="mt-1.5 h-11 border-gray-200 text-sm" value={formData.name} onChange={e => set("name", e.target.value)} placeholder="e.g. Netflix" required />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="number" min="0" step="0.01" placeholder="0.00" className="mt-1.5 h-11 border-gray-200 text-sm" value={formData.price} onChange={e => set("price", e.target.value)} required />
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-700">Billing cycle</Label>
                  <Select value={formData.billing_cycle} onValueChange={v => set("billing_cycle", v)}>
                    <SelectTrigger className="mt-1.5 h-11 border-gray-200 text-sm"><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div>
                ...
                <Input type="date" className="mt-1.5 h-11 border-gray-200 text-sm" value={formData.next_billing_date} onChange={e => set("next_billing_date", e.target.value)} required />
              </div>

              <div>
                <Label className="text-sm font-medium text-gray-700">Notes <span className="text-gray-400 font-normal">(optional)</span></Label>
                <Input placeholder="Plan details, account email..." className="mt-1.5 h-11 border-gray-200 text-sm" value={formData.notes} onChange={e => set("notes", e.target.value)} />
              </div>

              <Button
                type="submit" form="sub-form" disabled={saving}
                className="w-full h-12 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold text-sm mt-2 capitalize"
              >
                {saving ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Saving...</> : editItem ? "Save changes" : `Add ${sectionLabel}`}
              </Button>
            </form>
          )}
        </div>
  ...
  return (
    <Dialog open onOpenChange={v => { if (!v) onCancel(); }}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader><DialogTitle>Remove &quot;{name}&quot;?</DialogTitle></DialogHeader>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button className="flex-1 h-11 bg-red-600 hover:bg-red-700 text-white rounded-xl" onClick={onConfirm}>Remove</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          <div className="text-right flex-shrink-0">
            ...
            <p className="text-[11px] text-gray-400">
              /{s.billing_cycle === "monthly" ? "mo" : s.billing_cycle === "yearly" ? "yr" : s.billing_cycle === "weekly" ? "wk" : "qtr"}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 mb-3">
          ...
          {s.billing_cycle === "yearly" && (
            <span className="text-[11px] text-gray-400">≈ {fmt(Math.round(s.price / 12))}/mo</span>
          )}
        </div>
...
export default function SubscriptionsPage() {
  ...
  // Split by section
  const subItems  = subscriptions.filter(s => SUBSCRIPTION_CATS.includes(s.category as CategoryKey));
  const billItems = subscriptions.filter(s => BILL_CATS.includes(s.category as CategoryKey));
  const sectionItems = section === "subscriptions" ? subItems : billItems;

  // Category filter within section
  const activeFilter    = section === "subscriptions" ? subFilter : billFilter;
  const setActiveFilter = section === "subscriptions" ? setSubFilter : setBillFilter;

  const filtered = activeFilter === "all"
    ? sectionItems
    : sectionItems.filter(s => s.category === activeFilter);

  const sectionCats = section === "subscriptions" ? SUBSCRIPTION_CATS : BILL_CATS;
  const filterTabs = [
    { key: "all", label: `All (${sectionItems.length})` },
    ...CATEGORIES
      .filter(c => sectionCats.includes(c.value) && sectionItems.some(s => s.category === c.value))
      .map(c => ({ key: c.value, label: c.label })),
  ];

  const totalMonthly    = sectionItems.reduce((sum, s) => sum + monthlyEquiv(s.price, s.billing_cycle), 0);
  const allTotalMonthly = subscriptions.reduce((sum, s) => sum + monthlyEquiv(s.price, s.billing_cycle), 0);
  const dueSoon         = sectionItems.filter(s => { const d = daysUntil(s.next_billing_date); return d >= 0 && d <= 7; }).length;

  const sectionLabel     = section === "subscriptions" ? "subscription" : "bill";
  const sectionLabelCap  = section === "subscriptions" ? "Subscription" : "Bill";
  const SectionIcon      = section === "subscriptions" ? Tv : Receipt;
  ...
  return (
    <div className="space-y-5">
        ...
        <div>
          ...
          {!loading && subscriptions.length > 0 && (
            <p className="text-gray-500 text-sm mt-0.5">
              {fmt(Math.round(allTotalMonthly))}/month total
            </p>
          )}
        </div>
        <button
      ...
      <div className="flex bg-gray-100 rounded-2xl p-1 gap-1">
        {([
          { key: "subscriptions" as SectionKey, label: "Subscriptions", count: subItems.length, icon: Tv },
          { key: "bills"        as SectionKey, label: "Bills",          count: billItems.length, icon: Receipt },
          ...
          >
            ...
            {label}
            <span className={cn("text-[11px] font-bold px-1.5 py-0.5 rounded-full", section === key ? "bg-blue-100 text-blue-700" : "bg-gray-200 text-gray-500")}>
              {count}
            </span>
          </button>
        ))}
      ...
      {!loading && sectionItems.length === 0 && (
        <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center py-20 text-center px-6">
          ...
          <h3 className="text-lg font-bold text-gray-900 mb-2">
            No {section === "subscriptions" ? "subscriptions" : "bills"} yet
          </h3>
          <p className="text-sm text-gray-500 mb-6 max-w-xs">
            {section === "subscriptions"
              ? "Add Netflix, Spotify, Adobe, or any digital service you pay for."
              : "Add rent, WiFi, electricity, credit card bills, and loan payments."}
          </p>
          <button
            onClick={openAdd}
        ...
        <>
          ...
          <div className="bg-gray-900 rounded-2xl p-5 mt-2">
            <p className="text-xs text-gray-400 mb-1">
              {sectionLabelCap} spend (monthly)
            </p>
            <p className="text-3xl font-bold text-white">{fmt(Math.round(totalMonthly))}</p>
            <p className="text-sm text-gray-400 mt-1">
              {fmt(Math.round(totalMonthly * 12))}/year · {sectionItems.length} {section}
            </p>
          </div>
        </>
      )}
      ...
      {/* Dialogs */}
      <AddDialog open={dialogOpen} onOpenChange={setDialogOpen} editItem={editItem} onSaved={fetchSubs} mode={section} />
      {deleteItem && (
        <DeleteDialog name={deleteItem.name} onConfirm={() => handleDelete(deleteItem)} onCancel={() => setDeleteItem(null)} />
      )}
      ...
      <div className="fixed bottom-6 right-4 lg:hidden">
        <button
          onClick={openAdd}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-sm font-semibold px-5 h-13 py-3.5 rounded-full shadow-xl shadow-blue-300 transition-colors"
